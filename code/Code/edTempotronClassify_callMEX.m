function [weights, correctRate, CorrectFireRate, CorrectNonFireRate,epoch]= ...
    edTempotronClassify_callMEX(weights, IsTraining, SimWhichSet, datafolder, MAXEPO, lmd,targetRate)
...
%             IsTraining,lmd,mu,weights,dw_Past);
    ...
    if isequal(SimWhichSet,'training set')
        ...
        if mexBuiltOnTrnSet ==0
           ...
           % codegen edTempotronCore -args {nNeuronPerOutput,nOutputs,T,SpkTimings,Addresses,TimeChnlLbl,NeuronsTgt,NeuronsOut,use_single_exponential,tau1,V0,lut1,lut2,dt,V_thr,numCorrectSlices,numTotSlices,numCorrectFireSlices,numTotFireSlices,numTotNonFireSlices,numCorrectNonFireSlices,IsTraining,lmd,mu,weights,dw_Past} -o edTempotronCore_onTrnSet_mex
            mexBuiltOnTrnSet = 1;
        end
        % call mex function:
%         [NeuronsTgt,NeuronsOut,numCorrectSlices,numTotSlices,numCorrectFireSlices,...
...
%             IsTraining,lmd,mu,weights,dw_Past);
    ...
    elseif isequal(SimWhichSet,'testing set')
        ...
        if mexBuiltOnTstSet==0
            ...
            %codegen edTempotronCore -args {nNeuronPerOutput,nOutputs,T,SpkTimings,Addresses,TimeChnlLbl,NeuronsTgt,NeuronsOut,use_single_exponential,tau1,V0,lut1,lut2,dt,V_thr,numCorrectSlices,numTotSlices,numCorrectFireSlices,numTotFireSlices,numTotNonFireSlices,numCorrectNonFireSlices,IsTraining,lmd,mu,weights,dw_Past} -o edTempotronCore_onTstSet_mex
            mexBuiltOnTstSet = 1;
        end
        % call mex function:
%         [NeuronsTgt,NeuronsOut,numCorrectSlices,numTotSlices,numCorrectFireSlices,...
